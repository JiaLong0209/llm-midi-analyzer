...
# Verify GPU setup
poetry run python -c "import torch; print(torch.cuda.get_device_name())"
```

**Dependencies:** `torch`, `symusic`, `pretty_midi`, `h5py`, `scikit-learn`, `transformers`, `peft`, `bitsandbytes`
...
```bash
cd /home/jialong/Programming/TKU_Assignments/114-2/NLP

# Step 1: Pre-tokenize the LMD dataset (run once, ~12 minutes)
cd llm-midi-analyzer
poetry run python src/preprocess_midi.py \
  --input data/lmd_matched --output data/tokenized_8d

# Step 2: Train VQ-VAE
poetry run python src/trainer/vqvae_trainer.py --mode best8d \
  --data_dir data/tokenized_8d --hidden_dim 512 --epochs 100 \
  --model_name omni_v1 --kmeans

# Step 3: Inspect reconstruction quality
poetry run python src/inspect_vqvae.py \
  --checkpoint checkpoints/best/omni_v1_epoch100.pt \
  --data-cache data/lmd_8d_best_cache.h5

# Step 4: Train LLM adapter
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
poetry run python src/train_adapter.py \
  --mode vqvae --data_dir data/tokenized_8d \
  --llm models/MIDI-LLM \
